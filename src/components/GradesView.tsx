...
function CoursePicker({
  ...
  return (
    <div className="gv-picker">
      ...
      {entries.length === 0 && (
        <p className="gv-picker-empty">
          No courses yet — add them in Settings.
        </p>
      )}
      {entries.map(([id, c]) => {
...
function Gradebook({
  ...
  function updateField(id: string, field: keyof Assignment, raw: string) {
    update(
      assignments.map((a) => {
        ...
        if (field === "weight")
          return { ...a, weight: Math.max(0, Number(raw) || 0) };
        if (field === "max_score")
          return { ...a, max_score: Math.max(1, Number(raw) || 100) };
        if (field === "earned") {
          const v = raw.trim();
          return {
            ...a,
            earned: v === "" ? null : Math.max(0, Number(v) || 0),
          };
        }
        return a;
      }),
  ...
  function addRow() {
    update([
      ...assignments,
      { id: uid(), name: "", weight: 10, earned: null, max_score: 100 },
    ]);
  }

  function removeRow(id: string) {
    if (id.startsWith("exam-")) return;
        ...
        <div>
            ...
            <div className="gv-stat-row">
              <div className="gv-stat">
                <div className="gv-stat-num">
                  {avg.toFixed(1)}
                  <span className="gv-stat-pct">%</span>
                </div>
          ...
          {avg == null && assignments.length === 0 && (
            <p className="gv-empty-hint">
              Add your assignments below and enter scores as grades come in.
            </p>
          )}
        </div>
      ...
      <div className="gv-scroll">
        {assignments.length > 0 && (
          <div className="gv-table">
            ...
            {assignments.map((a) => {
              const p = pct(a);
              const isParsedExam = a.id.startsWith("exam-");
              return (
                <div
                  key={a.id}
                  className={`gv-row${a.earned !== null ? " gv-row-done" : ""}`}
                >
                  <input
                    ...
                    readOnly={isParsedExam}
                    title={
                      isParsedExam
                        ? "Parsed exam entries are locked"
                        : undefined
                    }
                  />
                  <input
                    ...
                    value={a.weight}
                    onChange={(e) =>
                      updateField(a.id, "weight", e.target.value)
                    }
                    readOnly={isParsedExam}
                  />
                  <input
                    ...
                    value={a.earned ?? ""}
                    onChange={(e) =>
                      updateField(a.id, "earned", e.target.value)
                    }
                    placeholder="—"
                  />
                  <input
                    ...
                    value={a.max_score}
                    onChange={(e) =>
                      updateField(a.id, "max_score", e.target.value)
                    }
                    readOnly={isParsedExam}
                  />
                  ...
                  {isParsedExam ? (
                    <span
                      className="gv-remove-btn"
                      title="Parsed exam entries cannot be removed"
                    >
                      -
                    </span>
                  ) : (
...
export default function GradesView({
  ...
  // Seed exam events into grade_assignments and reconcile stale auto-seeded exam rows.
  useEffect(() => {
    ...
    for (const [courseId, c] of Object.entries(courses)) {
      const examEvs = events.filter(
        (ev) =>
          isExamEvent(ev.summary) &&
          matchToCourse(ev.summary, { [courseId]: c }) !== null,
      );

      promises.push(
        loadAssignments(courseId).then((rows) => {
          const manual = rows.filter((r) => !r.id.startsWith("exam-"));
          const existingExam = new Map(
            rows
              .filter((r) => r.id.startsWith("exam-"))
              .map((r) => [r.id, r] as const),
          );

          const desiredExamRows = examEvs.map((ev) => {
            const id = `exam-${ev.uid}`;
            const existing = existingExam.get(id);
          ...
          const merged = [...manual, ...desiredExamRows];

          const sameLength = merged.length === rows.length;
          const unchanged =
            sameLength &&
            merged.every((a, i) => {
              const b = rows[i];
      ...
      {course && selectedId ? (
        ...
        <div className="empty-pane">
          ...
          <div className="empty-title">
            {ids.length === 0
              ? "No courses yet — complete setup first"
