...
fn verify_node_version(node: &Path) -> Result<(), String> {
    ...
    if !output.status.success() {
        return Err(format!(
            "{} --version failed; install NodeJs 18+ and make it available on path",
            node.display()
        ));
    }
    ...
    let Some(major) = node_major_version(&raw_version) else {
        return Err(format!(
            "Cannot parse NodeJs version from '{}'",
            raw_version.trim()
        ));
    };
    ...
    if major < 18 {
        return Err(format!(
            "NodeJs 18+ is required for mizu MCP tools; found {}",
            raw_version.trim()
        ));
    }
...
fn mcp_server_dir_candidates(
    ...
    if let Some(exe_dir) = exe_path.parent() {
        ...
        if let Some(bundle_root) = exe_dir.parent() {
            ...
            candidates.push(bundle_root.join("lib").join("mizu").join("mcp-server"));
        }
    }
